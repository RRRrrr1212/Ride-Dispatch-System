# 🚗 Ride-Dispatch-System (共乘/叫車平台)

> 模擬 Uber/Taxi 的核心派單與調度邏輯系統

## 📋 專案概述

本專案為共乘叫車平台的核心邏輯模擬系統，重點實作：
- **配對演算法** - 最近距離、車種篩選
- **計價系統** - 參數化費率公式
- **狀態機** - 訂單生命週期管理
- **併發控制** - H2 搶單、H4 冪等性

## 🏗️ 系統架構

```
┌─────────────────────────────────────────────────────────────┐
│                   JavaFX Clients (三視窗)                    │
├──────────────┬──────────────────┬───────────────────────────┤
│ Passenger App│   Driver App     │      Admin Console        │
└──────┬───────┴────────┬─────────┴─────────────┬─────────────┘
       │                │                       │
       │         HTTP/REST (Polling)            │
       │                │                       │
┌──────▼────────────────▼───────────────────────▼─────────────┐
│                   Spring Boot Server                         │
└─────────────────────────────────────────────────────────────┘
```

### 通訊方式
- **協定**: HTTP/REST
- **資料格式**: JSON
- **即時同步**: Polling (每 0.5-1 秒輪詢)

## 📁 專案結構

```
ride-dispatch-system/
├── server/                    # Spring Boot 後端
│   ├── src/main/java/
│   │   └── com/uber/
│   │       ├── controller/    # REST API Controllers
│   │       ├── service/       # Business Logic
│   │       ├── model/         # Domain Models
│   │       └── repository/    # Data Access
│   └── pom.xml
│
├── clients/                   # JavaFX 前端
│   ├── passenger-app/         # 乘客端視窗
│   ├── driver-app/            # 司機端視窗
│   └── admin-console/         # 管理後台視窗
│
├── docs/                      # 文件
│   ├── state-machine.md       # 狀態機合約 ⭐
│   ├── api-spec.md            # API 規格 ⭐
│   └── SYSTEM_SPEC.md         # 系統規格書
│
└── README.md
```

## 🚀 快速開始

### 環境需求
- Java 17+
- Maven 3.9+
- JavaFX 17+

### 啟動後端
```bash
cd server
mvn spring-boot:run
```

### 啟動前端 (三視窗)
```bash
cd clients
mvn javafx:run -pl passenger-app
mvn javafx:run -pl driver-app
mvn javafx:run -pl admin-console
```

## 🧪 測試重點

### H2: 搶單併發測試
```
場景: 10 位司機同時對同一筆 PENDING 訂單發送 Accept
預期: 僅 1 位成功 (200 OK)，其餘 9 位失敗 (409 Conflict)
驗證: 最終 driverId 唯一，Audit Log 記錄完整
```

### H4: 冪等性測試
```
場景: 同一請求因網路問題被重複發送
預期: 重複請求不產生副作用
驗證: driverId、startAt、fare 等關鍵欄位不被覆寫
```

### 執行測試
```bash
# 執行所有測試
mvn test

# 產生覆蓋率報告
mvn jacoco:report

# 產生 PMD 報告
mvn pmd:pmd
```

## 📖 核心文件

| 文件 | 說明 | 組員用途 |
|-----|------|---------|
| [docs/state-machine.md](docs/state-machine.md) | 狀態機合約 | 撰寫狀態機測試 |
| [docs/api-spec.md](docs/api-spec.md) | API 規格 | 撰寫整合測試 |
| [docs/SYSTEM_SPEC.md](docs/SYSTEM_SPEC.md) | 系統規格書 | 報告撰寫參考 |

## 🔧 開發指南

### API 端點總覽

#### Passenger API
| Method | Endpoint | 說明 |
|--------|----------|-----|
| POST | `/api/orders` | 建立叫車 |
| GET | `/api/orders/{id}` | 查詢狀態 |
| PUT | `/api/orders/{id}/cancel` | 取消訂單 |

#### Driver API
| Method | Endpoint | 說明 |
|--------|----------|-----|
| PUT | `/api/drivers/{id}/online` | 上線 |
| GET | `/api/drivers/{id}/offers` | 取得可接單 |
| PUT | `/api/orders/{id}/accept` | 接單 |
| PUT | `/api/orders/{id}/start` | 開始行程 |
| PUT | `/api/orders/{id}/complete` | 完成行程 |

#### Admin API
| Method | Endpoint | 說明 |
|--------|----------|-----|
| GET | `/api/admin/orders` | 所有訂單 |
| GET | `/api/admin/audit-logs` | Audit Log |

## 📊 測試覆蓋率目標

| 指標 | 目標 |
|-----|-----|
| Line Coverage | ≥ 80% |
| Branch Coverage | ≥ 70% |
| Method Coverage | ≥ 75% |

## 👥 團隊分工

| 角色 | 負責項目 |
|-----|---------|
| 後端開發 | Spring Boot Server、核心邏輯 |
| 前端開發 | JavaFX 三視窗 |
| 測試 | JUnit 單元/整合/併發測試 |

---

**版本**: v0.1  
**更新日期**: 2025-12-25
